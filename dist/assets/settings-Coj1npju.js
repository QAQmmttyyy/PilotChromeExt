...
只输出可执行 JavaScript。`;async function*O(t,e){const i=await fetch(e.endpoint||g,{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${e.apiKey}`,"HTTP-Referer":"chrome-extension://pilot","X-Title":"Pilot Chrome Extension"},body:JSON.stringify({model:e.model,messages:[{role:"system",content:f},...t],stream:!0})});if(!i.ok){const a=await i.json().catch(()=>({error:{message:i.statusText}}));throw new Error(a.error?.message||`请求失败: ${i.status}`)}const n=i.body?.getReader();if(!n)throw new Error("无法读取响应流");const r=new TextDecoder;let o="";for(;;){const{done:a,value:T}=await n.read();if(a)break;o+=r.decode(T,{stream:!0});const d=o.split(`
`);o=d.pop()||"";for(const u of d)if(u.startsWith("data: ")){const l=u.slice(6);if(l==="[DONE]")return;try{const h=JSON.parse(l).choices?.[0]?.delta?.content;h&&(yield h)}catch{}}}}function A(t,e){return e?e.startsWith("<<RECORDING_CONTEXT>>")?`${e}
