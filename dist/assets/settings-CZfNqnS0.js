...
用户需求:
${t}`:t}function E(t,e){const i=[];if(i.push(`Step ${e+1}: ${v(t.type)}`),i.push(`- URL: ${t.url}`),i.push(`- 页面: ${t.pageTitle}`),t.element){const n=t.element,o=Object.entries(n.attributes).map(([r,s])=>`${r}="${s}"`).join(" ");i.push(`- 元素: <${n.tag}${o?" "+o:""}>${n.text}</${n.tag}>`),i.push(`- 选择器候选: ${n.selectors.slice(0,5).join(", ")}`)}return t.value!==void 0&&(t.type==="ai_step"?i.push(`- AI 指令: ${JSON.stringify(t.value)}`):i.push(`- 输入值: "${t.value}"`)),t.key&&i.push(`- 按键: ${t.key}`),i.join(`
`)}function v(t){return{click:"点击",input:"输入",navigate:"页面导航",submit:"提交表单",select:"选择下拉框",keypress:"按键",ai_step:"AI 指令"}[t]||t}function A(t){const e=[];e.push("<<RECORDING_CONTEXT>>"),e.push(`录制名称: ${t.name}`),e.push(`起始 URL: ${t.startUrl}`),e.push(`步骤数量: ${t.steps.length}`),e.push(""),e.push("## 录制的操作流程"),e.push("");let i="";return t.steps.forEach((n,o)=>{n.url!==i&&(i&&e.push(""),e.push(`### 页面: ${n.url}`),i=n.url),e.push(""),e.push(E(n,o))}),e.push(""),e.push("<<END_RECORDING_CONTEXT>>"),e.join(`
`)}function w(t){let e=t;e=e.replace(/^```(?:javascript|js)?\s*\n?/i,""),e=e.replace(/\n?```\s*$/i,"");const i=["// === STEP:","(async () =>",";(async () =>","(() =>",";(() =>","const ","let ","var ","function "];let n=-1;for(const o of i){const r=e.indexOf(o);r>=0&&(n===-1||r<n)&&(n=r)}return n>0&&(e=e.slice(n)),e.trim()}function k(t){return m.find(e=>e.id===t)}const p="pilot_settings",c={ai:{apiKey:"",model:m[0].id,endpoint:"https://openrouter.ai/api/v1/chat/completions"},agentServerUrl:"http://localhost:3000"},a={get:async()=>{const e=(await chrome.storage.local.get(p))[p];return{ai:{apiKey:e?.ai?.apiKey??c.ai.apiKey,model:e?.ai?.model??c.ai.model,endpoint:e?.ai?.endpoint??c.ai.endpoint},agentServerUrl:e?.agentServerUrl??c.agentServerUrl}},set:async t=>{const e=await a.get(),i={ai:{apiKey:t.ai?.apiKey??e.ai.apiKey,model:t.ai?.model??e.ai.model,endpoint:t.ai?.endpoint??e.ai.endpoint},agentServerUrl:t.agentServerUrl??e.agentServerUrl};await chrome.storage.local.set({[p]:i})},getAIConfig:async()=>(await a.get()).ai,setAIConfig:async t=>{const e=await a.get();await a.set({ai:{apiKey:t.apiKey??e.ai.apiKey,model:t.model??e.ai.model,endpoint:t.endpoint??e.ai.endpoint}})},hasApiKey:async()=>!!(await a.get()).ai.apiKey,getServerUrl:async()=>(await a.get()).agentServerUrl},x=a.get;export{m as A,x as a,A as b,w as c,O as d,y as e,k as g,a as s};
