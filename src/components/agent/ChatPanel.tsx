...
export function ChatPanel({ conversationId }: ChatPanelProps) {
  ...
  useEffect(() => {
    getSettings().then(settings => {
      setServerUrl(settings.agentServerUrl || 'http://localhost:3000');
    });
  }, []);
    ...
    if (lastMsg?.role === 'assistant' && lastMsg.parts) {
      lastMsg.parts.forEach(async (part: any) => {
        if (isToolPart(part)) {
          ...
          if (toolName === 'executeWorkflow' && part.state === 'output-available') {
            if (executedToolIds.has(part.toolCallId)) return;
            
            setExecutedToolIds(prev => new Set(prev).add(part.toolCallId));
            
            const result = part.output as { script?: string, action?: string };
            if (result?.script) {
              try {
